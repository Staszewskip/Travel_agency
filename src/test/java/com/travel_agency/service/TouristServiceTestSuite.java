...
class TouristServiceTestSuite {
    ...
    @Test
    void findTourist() throws TouristNotFoundException {
        // Given
        Tourist tourist = new Tourist("tourist", "lastname", true, "login", "password", "email", 123456);
        // When
        touristRepository.save(tourist);
        Long touristId = tourist.getTouristId();
        TouristDTOGet foundTourist = touristService.findTourist(touristId);
        // Then
        assertEquals(tourist.getFirstname(),foundTourist.firstname());
        // Cleanup
        touristRepository.deleteAll();
    }
    ...
    void findAllTourists() {
        ...
        long size = touristService.findAllTourists().size();
        // Then
        assertEquals(2, size);
        // Cleanup
        touristRepository.deleteAll();
